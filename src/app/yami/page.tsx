...
export default async function Yami({ searchParams }: any) {
  ...
  if (!token) {
    let ct = "";
    const tcode = await fetch("https://apiw3.eaeja.com/vw3/code", {
      cache: "no-store",
    }).then((res) => {
  ...
  if (!id && !sid && !cid && !gid && !q && !sort) {
    const { categorys }: { categorys: Category[] } = await ff(
      "https://apiw3.eaeja.com/vw3/categorys",
      {},
      token
    );
  ...
  if (q) {
    const {
      ...
      "https://apiw3.eaeja.com/vw3/search",
      {
        actor_type: "long",
        next: to,
        q,
  ...
  if (cid) {
    const {
      ...
      `https://apiw3.eaeja.com/vw3/lite/category/${cid}/videos`,
      {
        video_type: "long",
        next: to,
      },
  ...
  if (gid) {
    const {
      ...
      `https://apiw3.eaeja.com/vw3/genre/${gid}/videos`,
      {
        video_type: "long",
        next: to,
      },
  ...
  if (sid) {
    const {
      ...
      `https://apiw3.eaeja.com/vw3/actor/${sid}/videos`,
      {
        actor_type: "long",
        next: to,
      },
  ...
  if (sort) {
    const girls: { actors: Actor[]; next: number } = await fetch(
      "https://apiw3.eaeja.com/vw3/category/actors",
      {
        method: "POST",
        body: JSON.stringify({
          actor_type: "long",
  ...
  const res: resProps = await ff(
    `https://apiw3.eaeja.com/vw3/video/${id}`,
    {
      cdn: "tc",
    },
    token
